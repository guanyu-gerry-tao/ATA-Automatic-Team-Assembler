...
class Student:
    def __init__(self,
        ...
        self.uid = None # TODO: add uid function
...
class Team:
    def __init__(self, students: list[Student]):
        ...
        self.have, self.want = np.array([]), np.array([])
    ...
    def __generate_have_want(self):
        ...
        if len(self.students) > 0:  # handle the case when there is at least one student in the team
            list_of_have = [student.have for student in self.students]  # init a list of have vector for each student
            list_of_want = [student.want for student in self.students]  # init a list of want vector for each student
            self.have = np.add.reduce(list_of_have) / len(list_of_have)  # average have vector
            ...
            self.have = np.array([])  # set have vector to be empty
            self.want = np.array([])  # set want vector to be empty
    ...
    def mutual_crush_score_with_rest_of_students(self, students_not_in_team: list[Student]):
        ...
        for student in students_not_in_team:
            score_be_like = np.dot(self.have, student.want)
            score_like = np.dot(self.want, student.have)
...
class Course:
    def __init__(self, semester: int, course_code: str, num_of_group: int):
    ...
    def __generate_have_want_arrays(self):
        ...
        for student in self.students:
            array_have.append(student.have)
            array_want.append(student.want)
    ...
    def team_matching(self):
        '''
        This is the actual team matching algorithm.
        :return:
        '''
        if self.num_of_group > len(self.students):  # handle the case when number of group is larger than number of students
        ...
        k = 0  # init a counter
        while k < self.num_of_group:  # while there are still teams left to be formed
            if len(self.student_not_in_team) + len(self.teams) == self.num_of_group:
                ...
                while self.student_not_in_team:
                    ...
                    self.teams.append(Team([student]))
                break
            ...
            if current_team[0] in self.student_not_in_team and current_team[1] in self.student_not_in_team:
                ...
                self.teams.append(Team([current_team[0], current_team[1]]))
                k += 1
...
class ExistedAnswers(BaseModel):
    email: set[str]
    native_language: set[str]
    skill_language: set[str]
    skill_other: set[str]
    background: set[str]
    hobby: set[str]
